 ...
 */
public class HotKeysButton extends Button{
	 ...
	 */
	public HotKeysButton(ActionNode action, ArrayList<ActionNode> historyList) {
		super();
		...
		String buttonInfo;
		buttonInfo = this.popUpTextInput();		
		if (buttonInfo.isBlank()) {
		...
		this.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				if (Control.isDeleteModeOn()) {
					((Pane)HotKeysButton.this.getParent()).getChildren().remove(HotKeysButton.this);
				}else {
					if (HotKeysButton.this.action.getNextActions().size() == 0) {
						System.out.println("end game");
						HotKeysButton.this.action.showAvaPos();
					} else {
						if (HotKeysButton.this.action instanceof FoldNode) { // Free Memory
							HotKeysButton.this.action.getEachPosWeight().setPosStackToNull(HotKeysButton.this.action.getPosition());
						}
						Control.setHistoryList(HotKeysButton.this.historyList);
						Control.setCurrActionNode(HotKeysButton.this.action);
						Control.setUpPane();
					}
				}
			}
